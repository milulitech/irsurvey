...
import math
image = Image.open("irsurvey/thermal/scale271.jpg")
palette = []
...
for j in range(len(palette) - 1):
	T.append(T[j] + step)
black_template = cv2.imread('irsurvey/visual/Black_template.jpg')
...
thermal = cv2.imread("irsurvey/visual/img_new3.jpg")
thermal = cv2.cvtColor(thermal, cv2.COLOR_BGR2RGB)
img  = Image.open("from PIL import Image
import numpy as np
import matplotlib.pyplot as plt
import cv2
from scipy.spatial import distance
...
for j in range(len(palette) - 1):
	T.append(T[j] + step)
black_template = cv2.imread('irsurvey/thermal/Black_template.jpg')
w, h = black_template.shape[::-1]
gray_template = cv2.cvtColor(black_template, cv2.COLOR_BGR2GRAY)
histogram_template = cv2.calcHist([gray_template], [0], None, [256], [0, 256])
thermal = cv2.imread("irsurvey/visual/img_new3.jpg")
thermal = cv2.cvtColor(thermal, cv2.COLOR_BGR2RGB)
img  = Image.open("irsurvey/visual/img_new3.jpg")
